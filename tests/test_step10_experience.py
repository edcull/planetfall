...
from planetfall.engine.models import CharacterClass, Loyalty
...
class TestAwardMissionXP:
    def test_participation_xp(self, game_state):
        state = game_state
    ...
    def test_casualty_loses_survival_xp(self, game_state):
        state = game_state
        name = state.characters[0].name
        old_xp = state.characters[0].xp
    ...
    def test_leader_kill_bonus(self, game_state):
        state = game_state
        name = state.characters[0].name
        old_xp = state.characters[0].xp
    ...
    def test_not_deployed_gets_nothing(self, game_state):
        state = game_state
        old_xp = state.characters[0].xp
        events = award_mission_xp(state, [], [])
        assert state.characters[0].xp == old_xp

    def test_double_xp_note(self, game_state):
        state = game_state
        name = state.characters[0].name
    ...
    def test_forfeit_xp_note(self, game_state):
        state = game_state
        name = state.characters[0].name
...
class TestRollAdvancement:
    ...
    def test_basic_advancement(self, mock_table, game_state):
    ...
    @patch("planetfall.engine.steps.step10_experience.ADVANCEMENT_TABLE")
    def test_maxed_stat(self, mock_table, game_state):
        mock_table.roll_on_table.return_value = (
            RollResult(dice_type="d100", values=[50], total=50, label=""),
            TableEntry(low=41, high=60, result_id="combat_skill",
                      description="CS up", effects={"stat": "combat_skill", "max": 5, "bonus": 1}),
    ...
    def test_insufficient_xp(self, game_state):
        state = game_state
        char = state.characters[0]
        char.xp = 2
        events = roll_advancement(state, char.name)
...
class TestBuyAdvancement:
    def test_buy_stat(self, game_state):
        state = game_state
        char = state.characters[0]
        char.xp = 10
    ...
    def test_maxed_stat(self, game_state):
        state = game_state
        char = state.characters[0]
        char.xp = 10
        char.savvy = 5
    ...
    def test_invalid_stat(self, game_state):
        state = game_state
        char = state.characters[0]
        char.xp = 10
    ...
    def test_speed_first_bonus_is_2(self, game_state):
        from planetfall.engine.models import STARTING_PROFILES
        state = game_state
        char = state.characters[0]
        char.xp = 10
        # Reset speed to starting value so first-buy bonus applies
        char.speed = STARTING_PROFILES[char.char_class].speed
...
class TestAlternateAdvancement:
    def test_loyalty_increase(self, game_state):
        state = game_state
        char = state.characters[0]
        char.xp = 10
    ...
    def test_loyalty_already_max(self, game_state):
        state = game_state
        char = state.characters[0]
        char.xp = 10
    ...
    def test_scientist_research_points(self, game_state):
        state = game_state
        scientist = None
    ...
    def test_non_scientist_cannot_get_rp(self, game_state):
        state = game_state
    ...
    def test_scout_raw_materials(self, game_state):
        state = game_state
        scout = None
        for c in state.characters:
