...
from planetfall.engine.models import (
    ColonizationAgenda, GameState, Theory, Building,
)
...
class TestResearch:
    ...
    def test_available_theories_initial(self):
        state = _make_state()
    ...
    def test_invest_in_theory(self):
        state = _make_state()
    ...
    def test_invest_partial_theory(self):
        state = _make_state()
    ...
    def test_secondary_theory_requires_prerequisite(self):
        state = _make_state()
        available = get_available_theories(state)
    ...
    def test_unlock_application(self):
        state = _make_state()
        state.colony.resources.research_points = 10
    ...
    def test_application_with_morale_effect(self):
        state = _make_state()
        state.colony.resources.research_points = 10
    ...
    def test_milestone_application(self):
        state = _make_state()
        state.colony.resources.research_points = 10
        state.tech_tree.theories["social_theories"] = Theory(
            name="Social Theories", invested_rp=3, required_rp=3, completed=True
...
class TestBuildings:
    ...
    def test_available_buildings_initial(self):
        state = _make_state()
    ...
    def test_invest_in_building_partial(self):
        state = _make_state()
    ...
    def test_invest_in_building_complete(self):
        state = _make_state()
    ...
    def test_building_with_effects(self):
        state = _make_state()
        state.colony.resources.build_points = 10
    ...
    def test_milestone_building(self):
        state = _make_state()
    ...
    def test_raw_materials_conversion(self):
        state = _make_state()
    ...
    def test_per_turn_building_effects(self):
        state = _make_state()
        # No buildings -> no effects
        events = process_per_turn_buildings(state)
        assert len(events) == 0

    def test_reclaim_building(self):
        state = _make_state()
    ...
    def test_prerequisite_building(self):
        state = _make_state()
        available = get_available_buildings(state)
...
class TestMilestones:
    ...
    def test_apply_milestone_1(self):
        state = _make_state()
    ...
    def test_apply_milestone_7_triggers_end_game(self):
        state = _make_state()
        events = apply_milestone(state, 7)
        assert state.campaign.end_game_triggered

    def test_check_and_apply_new_milestones(self):
        state = _make_state()
...
class TestStepIntegration:
    ...
    def test_step14_get_options(self):
        state = _make_state()
    ...
    def test_step15_get_options(self):
        state = _make_state()
...
class TestNarrative:
    ...
    def test_generate_local_narrative(self):
        state = _make_state()
        from planetfall.engine.models import TurnEvent, TurnEventType
        events = [TurnEvent(
    ...
    def test_narrative_memory_updates(self):
        state = _make_state()
        from planetfall.engine.models import TurnEvent, TurnEventType
        events = [TurnEvent(
            step=8, event_type=TurnEventType.COMBAT,
