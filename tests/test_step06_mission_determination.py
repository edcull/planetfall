...
from planetfall.engine.models import (
    ColonizationAgenda, MissionType, SectorStatus, TacticalEnemy, TurnEvent,
    TurnEventType,
)
...
class TestAvailableMissions:
    ...
    def test_scouting_with_unexplored_sectors(self):
        state = _make_state()
        ...
        for s in state.campaign_map.sectors:
            if s.sector_id != state.campaign_map.colony_sector_id:
                s.status = SectorStatus.UNKNOWN
                break
        missions = get_available_missions(state)
        types = [m["type"] for m in missions]
        assert MissionType.SCOUTING in types

    def test_investigation_with_site(self):
        state = _make_state()
        for s in state.campaign_map.sectors:
            if s.sector_id != state.campaign_map.colony_sector_id:
    ...
    def test_exploration_with_explored_sectors(self):
        state = _make_state()
        for s in state.campaign_map.sectors:
            if s.sector_id != state.campaign_map.colony_sector_id:
    ...
    def test_hunt_with_lifeforms(self):
        state = _make_state()
    ...
    def test_skirmish_and_strike_with_enemies(self):
        state = _make_state()
    ...
    def test_assault_with_strongpoint(self):
        state = _make_state()
        state.enemies.tactical_enemies = [
            TacticalEnemy(
    ...
    def test_delve_with_ancient_site(self):
        state = _make_state()
        for s in state.campaign_map.sectors:
    ...
    def test_rescue_from_turn_log(self):
        state = _make_state()
        state.turn_log.append(TurnEvent(
