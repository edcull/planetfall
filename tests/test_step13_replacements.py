...
class TestVacancies:
    def test_full_roster_no_vacancies(self, game_state):
        state = game_state
        # Ensure full roster
        assert get_vacancies(state) == MAX_ROSTER_SIZE - len(state.characters)

    def test_vacancies_after_death(self, game_state):
        state = game_state
...
class TestExecute:
    def test_full_roster(self, game_state):
        state = game_state
        # Fill roster to max
    ...
    def test_has_vacancies(self, game_state):
        state = game_state
        state.characters.pop()
        events = execute(state)
